...
public interface CurriculaRepository extends JpaRepository<Curricula, Long> {
  ...
  @Query("""
          ...
          WHERE (:programTitle IS NULL OR :programTitle = '' OR c.programTitle = :programTitle)
            AND (:courseTitle IS NULL OR :courseTitle = '' OR
                 LOWER(c.courseTitle) LIKE LOWER(CONCAT('%', :courseTitle, '%')))
          ORDER BY c.courseTitle ASC
      """)
  List<Curricula> findByProgramTitleAndCourseTitle(
