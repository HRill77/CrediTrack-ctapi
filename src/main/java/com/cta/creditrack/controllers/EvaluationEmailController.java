...
public class EvaluationEmailController {
        ...
        public ResponseEntity<?> sendEmail(
                ...
                try {
                        CustomUserDetials userDetails = (CustomUserDetials) authentication.getPrincipal();

                        User sender = userRepository.findById(
                                        userDetails.getUser().getId())
                                        .orElseThrow(() -> new RuntimeException("User not found"));

                        log.info("Sending evaluation email for students: {} to: {}", request.studentIds(),
                                        request.recipientEmail());

                        // Pass the html field if it exists in the request
                        emailService.sendEvaluationEmail(
                                        request.studentIds(),
                                        request.recipientEmail(),
                                        sender,
                                        request.html() // ADD THIS LINE
                        );
